...
(() => {
    ...
    function unwrapSnap(o) {
        ...
        if (s && s.data && typeof s.data === "object")
            return s.data;
        if (s && (s.enabled || s.alive !== undefined || s.bots))
            return s;
        return o || {};
    }
    function BotPanel({ React, HTTP, useQuery }) {
        ...
        React.useEffect(() => { if (query.isError)
            setBlocked(true); }, [query.isError]);
        ...
        const act = async (body) => {
            ...
            try {
                await HTTP.post("/api/bot", body);
            }
            catch (e) { /* query refetch shows the real state */ }
            setBusy("");
        };
        const btn = (label, body, cls) => h("button", { className: "botmod-btn" + (cls ? " " + cls : ""), disabled: !!busy, onClick: () => act(body) }, label);
        const pill = (on, onLabel, offLabel) => h("span", { className: "botmod-pill " + (on ? "botmod-ok" : "botmod-off") }, on ? onLabel : offLabel);
        return h("div", { className: "botmod-panel" }, h("div", { className: "botmod-head" }, h("h2", null, "Bot Control"), pill(enabled, "ENABLED", "DISABLED"), h("span", { className: "botmod-window" }, "alive " + (s.alive || 0) + "/" + (s.targetBotCount || 0) + " · max " + (s.maxBots || 0) +
            " · diff " + (s.difficulty || 0) + " · " + (s.weapon || "?") + " · brain " +
            (s.neural ? (s.neuralLoaded ? "GA" : "GA (not loaded)") : "static AI"))), h("div", { className: "botmod-row" }, btn(enabled ? "Disable" : "Enable", { action: enabled ? "disable" : "enable" }, enabled ? "botmod-danger" : "botmod-primary"), btn("Remove all", { action: "remove" }, "botmod-danger"), h("input", {
            className: "botmod-num", type: "number", min: 1, max: 16, value: spawnCount,
            onChange: (e) => setSpawnCount(e.target.value)
        }), btn("Spawn bots", { action: "spawn", count: parseInt(spawnCount, 10) || 1 }, "botmod-primary")), h("div", { className: "botmod-row" }, h("span", { className: "botmod-label" }, "Near player:"), onlinePlayers.length === 0
        ...
        }, "botmod-primary")), h("div", { className: "botmod-row botmod-brain" }, h("span", { className: "botmod-label" }, "Brain:"), btn(s.neural ? "Static AI" : "GA brain", { action: "neural", on: !s.neural }), s.neuralPath ? h("span", { className: "botmod-window" }, "weights: " + s.neuralPath) : null), h("h3", null, "Scoreboard"), bots.length === 0
            ? h("p", { className: "botmod-empty" }, "No bots alive.")
            : h("table", { className: "botmod-table" }, h("thead", null, h("tr", null, ["Bot", "Weapon", "HP", "Kills P", "Kills Z", "Deaths", "Score", "Lvl", "Near", "State"].map((x) => h("th", { key: x }, x)))), h("tbody", null, bots.map((b) => h("tr", { key: b.entityId }, h("td", null, b.name), h("td", null, b.weapon), h("td", null, b.health), h("td", null, b.players), h("td", null, b.zombies), h("td", null, b.deaths), h("td", null, b.score), h("td", null, b.level), h("td", null, b.nearestPlayerDist !== undefined && b.nearestPlayerDist >= 0
