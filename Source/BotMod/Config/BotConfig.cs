    ...
    public struct WeaponProfile
    {
        ...
        public static WeaponProfile ForGun(string gunId, BotConfig cfg)
        {
            ...
            if (g.Contains("shotgun"))
            {
                ...
                return new WeaponProfile { GunId = gunId, FireRate = autoShot ? 0.22f : 0.55f, BurstMin = 1, BurstMax = 1, BurstPause = autoShot ? 0.4f : 0.85f, SpreadDeg = autoShot ? 6f : 9f, Damage = autoShot ? 9 : 14, Range = 22f, Pellets = autoShot ? 6 : 8, MagSize = 6, ReloadSec = 2.6f };
            }
            if (g.Contains("sniper") || g.Contains("hunting") || g.Contains("lever"))
                return new WeaponProfile { GunId = gunId, FireRate = 0.9f, BurstMin = 1, BurstMax = 1, BurstPause = 0.9f, SpreadDeg = 0.35f, Damage = 42, Range = 90f, Pellets = 1, MagSize = 5, ReloadSec = 2.5f };
            if (g.Contains("smg") || g.Contains("pipe") && g.Contains("machine"))
                return new WeaponProfile { GunId = gunId, FireRate = 0.09f, BurstMin = 5, BurstMax = 9, BurstPause = 0.5f, SpreadDeg = 2.2f, Damage = 9, Range = 35f, Pellets = 1, MagSize = 32, ReloadSec = 1.8f };
            ...
            // pistol default
            return new WeaponProfile { GunId = gunId, FireRate = 0.28f, BurstMin = 1, BurstMax = 3, BurstPause = 0.6f, SpreadDeg = 1.6f, Damage = 16, Range = 40f, Pellets = 1, MagSize = 12, ReloadSec = 1.2f };
        }
    }
}
