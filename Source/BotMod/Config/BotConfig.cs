    ...
    public struct WeaponProfile
    {
        ...
        public static WeaponProfile ForGun(string gunId, BotConfig cfg)
        {
            ...
            if (g.Contains("shotgun"))
            {
                ...
                return new WeaponProfile { GunId = gunId, FireRate = autoShot ? 0.22f : 0.55f, BurstMin = 1, BurstMax = 1, BurstPause = autoShot ? 0.4f : 0.85f, SpreadDeg = autoShot ? 6f : 9f, Damage = autoShot ? 9 : 14, Range = 22f, Pellets = autoShot ? 6 : 8 };
            }
            if (g.Contains("sniper") || g.Contains("hunting") || g.Contains("lever"))
                return new WeaponProfile { GunId = gunId, FireRate = 0.9f, BurstMin = 1, BurstMax = 1, BurstPause = 0.9f, SpreadDeg = 0.35f, Damage = 42, Range = 90f, Pellets = 1 };
            if (g.Contains("smg") || g.Contains("pipe") && g.Contains("machine"))
                return new WeaponProfile { GunId = gunId, FireRate = 0.09f, BurstMin = 5, BurstMax = 9, BurstPause = 0.5f, SpreadDeg = 2.2f, Damage = 9, Range = 35f, Pellets = 1 };
            if (g.Contains("m60") || g.Contains("tactical") || g.Contains("ak"))
                return new WeaponProfile { GunId = gunId, FireRate = 0.11f, BurstMin = 3, BurstMax = 6, BurstPause = 0.55f, SpreadDeg = 1.4f, Damage = 16, Range = 55f, Pellets = 1 };
            if (g.Contains("magnum") || g.Contains("desert"))
                return new WeaponProfile { GunId = gunId, FireRate = 0.32f, BurstMin = 1, BurstMax = 2, BurstPause = 0.6f, SpreadDeg = 1.0f, Damage = 34, Range = 45f, Pellets = 1 };
            // pistol default
            return new WeaponProfile { GunId = gunId, FireRate = 0.28f, BurstMin = 1, BurstMax = 3, BurstPause = 0.6f, SpreadDeg = 1.6f, Damage = 16, Range = 40f, Pellets = 1 };
        }
    }
}
