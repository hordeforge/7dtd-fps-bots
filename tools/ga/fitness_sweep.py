...
def one_mix(mix):
    ...
    # Held scoring always on the canonical measuring stick (apples-to-apples):
    # harness.canonical_scores pins FIT_*/DRAWS=1 (F=18) like evolve._held_probe
    # / eval_static_vs_neural and restores the training mix afterwards; leaving
    # the training values in scored held60 on a different sample than every
    # other tool.
    scores = harness.canonical_scores(best_w, 999, HOLD_SEED, 60)
